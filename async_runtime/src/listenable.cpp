...
void Listenable::addListener(Object::Ref<VoidCallBack> fn)
{
    assert(!this->_isDisposed);
    this->_listeners[(size_t)fn.get()] = fn;
}

void Listenable::removeListener(Object::Ref<VoidCallBack> fn)
{
    using iterator = std::unordered_map<size_t, Object::Ref<VoidCallBack>>::iterator;
